    ...
    let tabs = Tabs::new(titles)
        .block(
            Block::default()
                ...
                .title(Span::styled(" Cursor-API ", accent_magenta())),
        )
        .select(selected)
...
fn draw_dashboard(frame: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    let p = Paragraph::new(lines)
        .block(panel_block(" Cursor-API · service dashboard "))
...
fn draw_agent(frame: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    let banner = if alive {
        "Interactive Cursor agent chat (separate from /v1 headless jobs). Chat here while the API serves other clients."
    } else {
        "Agent not running — press r to start, or Shift+R to restart"
    };
    let banner_p = Paragraph::new(Span::styled(banner, muted()))
        .block(panel_block(" Cursor-API · Agent "));
...
fn draw_cli(frame: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    let banner_p = Paragraph::new(Span::styled(banner, muted()))
        .block(panel_block(" Cursor-API · CLI "));
    frame.render_widget(banner_p, chunks[0]);
