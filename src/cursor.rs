...
impl CursorBackend {
    ...
    pub async fn stream(
        ...
        let mut child = cmd
            ...
            .with_context(|| {
                format!(
                    "failed to spawn cursor agent for streaming ({})",
                    launch.display
                )
            })?;
...
pub fn resolve_agent_launch(configured: &str) -> Result<AgentLaunch> {
    if !configured.is_empty() {
        return launch_from_path(PathBuf::from(configured));
    }

    // Prefer the real bundled Node + index.js so we do not go through
    // Windows agent.cmd → PowerShell, or Unix wrapper scripts.
    if let Some(launch) = find_bundled_node_launch() {
        return Ok(launch);
    }

    for name in ["agent", "cursor-agent"] {
        if let Some(path) = which(name) {
            return launch_from_path(path);
        }
    }

    for candidate in common_agent_candidates() {
        if candidate.is_file() {
            return launch_from_path(candidate);
        }
    }

    bail!(
        "could not find Cursor `agent` CLI on PATH. Install from https://cursor.com/docs/cli/overview then run `agent login`"
    )
}
