...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development. User asked **not** to git commit unless requested â€” **do not commit**.

**Goal:** Ship polished Cursor-API (plugin ingest hub + TUI Agent/CLI tabs + token budget) for general OpenAI-compatible clients.
...
- Keep existing OpenAI `/v1/chat/completions` working for any OpenAI-compatible client on `/v1`
...
- Soft behavior: if prompt tokens estimate > max_context_tokens, log warn; optional truncate only if config `truncate_over_context = true` (default false for safety)
...
- Refactor chat completions to go: parse via openai_chat adapter â†’ runner
...
### Task 3: TUI tabs â€” reorder, token UX, Agent tab, CLI last

**Files:** `src/tui/*`

Tab order: Dashboard, Config, Logs, Usage, Agent, Help, CLI (keys 1â€“7 / F1â€“F7)
...
- Help updated
- Escape from Agent/CLI: F-keys, Ctrl+â†/â†’, Ctrl+Q

**Test:** `cargo build --release`; manual note in README

---

### Task 4: Rebuild, restart, smoke
...
- Confirm clients still point at `http://127.0.0.1:8787/v1`

---

## Execution order

1 â†’ 2 â†’ 3 â†’ 4 (sequential; shared files). No commits.
