...
#[command(
    name = "cursor-api",
    about = "Cursor-API — OpenAI-compatible /v1 bridge to the Cursor Agent CLI with a management TUI",
    version
)]
struct Cli {
    #[command(subcommand)]
...
enum Commands {
    ...
    ConfigPath,
    /// Check that the Cursor agent binary is reachable
    Doctor {
...
async fn main() -> Result<()> {
    ...
    match cli.command.unwrap_or(Commands::Tui {
        ...
        Commands::Serve => {
            ...
            println!(
                "tip: prefer `cursor-api` (TUI + API). `serve` is headless-only — no dashboard."
            );
