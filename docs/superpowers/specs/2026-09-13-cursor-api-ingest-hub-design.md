# Cursor-API â€” Ingest Hub + TUI Control Plane

**Status:** Approved (user 2026-09-13)  
**Goal:** Long-term local ingestion hub for Cursor Agent CLI + polished TUI so any client can use `/v1` painlessly.

## Decisions locked

| Decision | Choice |
|----------|--------|
| Agent tab vs `/v1` | **A â€” separate lanes** (headless `agent -p` vs interactive Agent PTY; shared login/model/workspace only) |
| Token UX | **C â€” both** settable max context tokens + live usage |
| `/v1` ambition | **Kitchen sink** via **plugin adapters from day one** |
| Architecture | Normalize-first hub: adapters â†’ `IngestRequest` â†’ one Runner |

## Â§1 Core architecture

```
Client â†’ HTTP router â†’ Adapter plugin â†’ IngestRequest â†’ Runner â†’ Cursor Agent CLI
TUI Agent tab â†’ separate interactive agent PTY (never shares /v1 session)
```
...
6. Help  
7. **CLI** (generic shell PTY â€” far right)

- Show **max context tokens** (config) on Dashboard + Config.
- Show **live / last usage** (promptâ‰ˆ, completionâ‰ˆ, total) on Dashboard + Usage.
- Agent tab: embed PTY running `agent` (or node+index.js) with workspace/model from config; Shift+R restart; F-keys / Ctrl+â†â†’ leave tab.
...
- `cursor.max_context_tokens` (default e.g. 128000) â€” advertised on `/health`, used for soft truncation warning / optional trim.
